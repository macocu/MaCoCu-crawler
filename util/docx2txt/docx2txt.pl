...
#    23/09/2008 - Changed the shebang line to use /usr/bin/env - good suggestion
#                 from Rene Maroufi (info>AT<maroufi>DOT<net) to reduce user
...
#Added by VS 2016-10-19
#Using time() to prevent opening the same temp file multiple times is wrong.
#This is a quick fix to allow multiple instances of docx2txt running
#concurrently. Increasing the time precision is still incorrect, tempfile()
#is the right way to prevent race conditions.
...
sub processParagraph {
    ...
    #Added by VS 2016-10-19
    #It enables to recognise paragraphs. Useful for cleaning by justext.
    $para = "$para\n";
